...
class RANEquipmentImageService {
  constructor() {
    this.uploadDir = path.join(__dirname, '../uploads/ran_equipment');
  ...
  generateUniqueFilename(originalName) {
    ...
    return `ran_equipment_${timestamp}_${rand}${ext}`;
  }
  ...
  async saveFile(file) {
    ...
    return { originalName: file.originalname, stored, filePath, fileUrl: `/uploads/ran_equipment/${stored}`, size: file.size, mime: file.mimetype };
  }

  async replaceImage({ file, session_id, image_category, description = null, metadata = {} }) {
    const existingImage = await RANEquipmentImages.findOne({
      where: { 
        session_id, 
        image_category,
        is_active: true 
    ...
    const newImage = await RANEquipmentImages.create({
      session_id,
      image_category,
      record_index: 1,
      original_filename: info.originalName,
  ...
  async getImagesBySessionId(sessionId) {
    return RANEquipmentImages.findAll({
      where: { 
  ...
  async deleteAllImagesBySessionId(sessionId) {
    const images = await RANEquipmentImages.findAll({
      where: { 
        session_id: sessionId,
        is_active: true 
      }
    ...
    const result = await RANEquipmentImages.update(
      { is_active: false },
      { 
        where: { 
          session_id: sessionId,
...
module.exports = new RANEquipmentImageService(); 